...
NosTale uses two sprite payload families. Map-object sprites contain one or
more descriptor-based `A4R4G4B4` frames. Free-size sprites contain one
...
Entries in `NSmpData*.NOS`, `NSppData*.NOS`, and `NSipData*.NOS` files
use the map-object format. The first byte is a frame count, followed by one
12-byte descriptor per frame and the referenced pixel blocks.
...
The source coordinates place the frame relative to its logical origin. They
are not crop coordinates. Known payloads place pixel blocks contiguously in
frame order immediately after the descriptor table, but readers must use each
descriptor's absolute offset.

A frame contains `width * height` row-major `A4R4G4B4` pixels. One
...
are typically splash screens, login backgrounds, and other large screen
images. Each payload contains one image and begins with a 4-byte header:
...
The image contains `width * height` `A8R8G8B8` pixels. Each little-endian
32-bit value has this layout:
