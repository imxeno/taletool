...
//! archive detection for supported container files, and reports a concise
//! classification.
...
/// Classify a single archive file using the same archive detection as commands.
...
fn is_scannable_archive(path: &Path) -> bool {
    path.extension()
        .and_then(|value| value.to_str())
        .map(|extension| {
