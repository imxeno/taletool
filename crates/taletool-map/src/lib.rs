...
//!
//! This crate currently supports the cell-flag grids stored in
//! `NStcData.NOS` archive entries.
...
impl MapCellGrid {
    ...
    pub fn new(width: u16, height: u16, cells: Vec<MapCellFlags>) -> Result<Self> {
...
pub fn decode_map_cell_grid(data: &[u8]) -> Result<MapCellGrid> {
    if data.len() < 4 {
        return Err(MapError::TooShort { actual: data.len() });
    }
    ...
    if actual != expected {
        return Err(MapError::InvalidPayloadLength { expected, actual });
    }
...
pub fn encode_map_cell_grid(grid: &MapCellGrid) -> Result<Vec<u8>> {
...
mod tests {
    ...
    fn rejects_invalid_payloads() {
        assert_eq!(
            ...
            Err(MapError::TooShort { actual: 3 })
        );
        assert!(matches!(
        ...
        assert_eq!(
            ...
            Err(MapError::InvalidPayloadLength {
                expected: 2,
                actual: 1
            })
        );
